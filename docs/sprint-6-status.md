...
Sprint 6F 跨场景角色任务迁移    已完成
Sprint 6G 音频强制对齐级评分    已完成工程入口
Sprint 6H 移动端深层截图回归    已完成
```
...
- 新增角色扮演迁移模块 `createRoleplayTransferPlan`。
- 根据当前角色场景长期记忆判断是否适合迁移到新场景。
- 迁移计划包含：
  - 准备度标签
  - 准备度分数
  - 迁移原则
  ...
  - 成功标准
  - 未达标时的锁定原因
- 练习页场景口语模块新增“跨场景迁移”卡片。
- 当前先提供迁移任务建议，不新增完整课程库，避免偏离“先完成功能”的目标。

验收记录：

- 新增 `tests/speech/roleplay-transfer.test.ts`。
- 测试覆盖：
  - 没有历史练习时锁定迁移任务
  - 当前场景足够稳定时解锁多个迁移目标
  - 中等完成度时只给小步迁移建议

## Sprint 6G：音频强制对齐级发音评分入口

已完成内容：

- 新增发音评分服务端模块 `scorePronunciation`。
- 新增 `/api/speech/pronunciation-score`，接收跟读录音、目标文本和转写文本。
- 支持 `PRONUNCIATION_PROVIDER=local` 接入本地 multipart 发音评分 endpoint。
- 支持配置 endpoint path、文件字段名、目标文本字段名、转写字段名、模型和超时时间。
- 本地服务返回后会归一化为总分、发音分、流利度、对齐分、词级评分和音素关注点。
- 练习页跟读录音完成后会请求音频级评分，并在跟读反馈区展示评分结果。
- 未配置本地发音评分时，练习页会明确显示降级原因，不影响原有文本级完整度和发音重点诊断。

说明：

- 这一轮完成的是“工程入口”和可配置 adapter。
- 生产级音素明细评分还需要后续选择或打包真正的强制对齐/发音评分模型。

验收记录：

- 新增 `tests/speech/pronunciation-score.test.ts`。
- 测试覆盖：
  - 默认未配置时走 fallback
  - 本地 provider 配置可解析 endpoint
  - 发音评分请求会发送 multipart audio
  - 本地返回可归一化为分数、词级反馈和音素关注点

## Sprint 6H：移动端深层截图回归

已完成内容：

- 新增 `docs/qa/mobile-deep-visual-regression-2026-06-01.md`。
- 新增 `docs/qa/screenshots/2026-06-01-mobile-deep/` 截图集。
- 覆盖移动端更长页面和深层入口：
  - 练习页首屏和长页面
  - 材料导入页
  - 词句本
  ...
  - 进度页长页面
  - 设置页长页面
- 发现并修复练习页跟读卡片在移动端的潜在横向溢出风险。
- 对 `/practice` 做 CDP 布局检查，确认修复后没有横向溢出元素。
...
- Sprint 7A：进入内容库和课程体系扩充。
- Sprint 7B：扩展美国生活、工作、移民场景材料和课程路径。
