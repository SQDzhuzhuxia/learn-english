...
import { materialFilters, materials } from "@/lib/mock-data";
      ...
      <section className="grid gap-5 lg:grid-cols-[1fr_340px]">
          ...
          <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
            <div>
              ...
              <p className="mt-3 max-w-3xl text-sm leading-6 text-muted">
                先选择略高于当前水平的材料。系统会优先推荐故事、生活对话、美国场景和自动化相关内容。
              </p>
            </div>
        ...
        <aside className="rounded-lg border border-border bg-panel p-5 shadow-sm">
          <div className="flex items-center justify-between">
            <h2 className="text-lg font-semibold text-foreground">今日推荐</h2>
          ...
          <article key={material.id} className="rounded-lg border border-border bg-panel p-5 shadow-sm">
            ...
            <div className="mt-4 grid grid-cols-3 gap-2 text-sm">
              ...
              <div>
                ...
                <p className="mt-1 font-semibold text-foreground">{material.knownRate}%</p>
              </div>
              <div>
      ...
      <section className="rounded-lg border border-border bg-panel p-5 shadow-sm">
        <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          <div>
            ...
            <h2 className="mt-2 text-lg font-semibold text-foreground">下一阶段会接入真实材料导入</h2>
            <p className="mt-2 text-sm leading-6 text-muted">
              Sprint 2 会优先做粘贴文本导入、自动分句、保存到材料库，并为后续字幕和音频转写预留结构。
            </p>
          </div>
          <button className="inline-flex min-h-11 items-center justify-center gap-2 rounded-lg border border-border bg-white px-4 py-2 text-sm font-semibold text-foreground hover:bg-panel-strong">
